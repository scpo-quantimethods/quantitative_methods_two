...
1. Download the dataset following <> and import it into your local R environment.
...
3. The dataset is in .stata format. You can inspect the different values a variable takes by calling: `attributes(data_set$variable)`. Do that for variable `first_round_vote`. Then, create three variables:
- `voteRN` = 1 if the respondent voted for RN in the first round, 0 otherwise (NA if no answer), 
- `voteREC`= 1 if the respondent voted for Reconquête in the first round, 0 otherwise (NA if no answer)
- `voteRR`= 1 if the respondent voted for either RN or Reconquête, 0 otherwise
Verify that the number of radical right voters equals the sum of RN and Reconquête voters. Using 2026 as the reference year, create a variable `age`.

4. Define a variable `female = 1` if the respondent identifies as a woman, 0 if male, and `NA_character_` otherwise (too few respondents in other categories or no answer).
- Clean `union_member, educ, religion`, and `income` by recoding non-responses as `NA`.
- Create a nominal variable `cohort` by grouping ages into decades: `<30, 30–39, 40–49, …, 80+`, then run 
```{r}
#| eval: false
...
7. Include `as.factor(educ)` in the regression. Compare models using `stargazer()` by adapting the code below, which exponentiates the coefficients from your logistic regressions:
```{r}
#| eval: false
...
```
Interpret the results: How does attending a “grande école” affect the odds of voting for the radical right? Is the coefficient on gender still significant ?

8. Include in the regression `religion, union_member`, and `income`. Only use as.factor(variable) for nominal variables with more than two categories. Disregard the warning : `Warning: glm.fit: fitted probabilities numerically 0 or 1 occurred`.
...
11. Bonus : Repeat the analysis for the second round of legislative elections. Compare the results with the first round in a text of 5 lines max. 
