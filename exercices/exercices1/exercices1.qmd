...
* Import the data in your local environment. Use the `rio` package and library to import it directly from the internet. 
* Use the glimpse() function to have a look at your data
* Filter the data set to only keep the most recent year, 2024.
* Using the identifiers and general information in the documentation, keep only the following items:"country", "the overall ideology on a scale ranging from 0 (extreme left) to 10 (extreme right)", "Thinking about the European Union's relationship with Russia, where do you place each party on its preferred action for European countries to take?", "Position on civil liberties VS. law and order", "Position on same sex marriage", "position on urban-rural", "position on nationalism", "left-right positioning". 
* Summarize three of these variables and comment on your findings.
* You now want to more specifically understand what drives a party to adopt favorable or more defavorable stances on the EU's relationship with Russia. What would be your dependent variable ? Plot a histogram of this DV and comment on the distribution.
* You want to understand if left-right positioning influences the EU-Russia position of a party:
...
* Now imagine wondering whether being in a country from Eastern or Western Europe influences how nationalism influences your position on this issue. How could you test that ? Plot the predicted values.
* Compute a last, fully saturated model, with all your variables except `country`, and run tests of multicollinearity and heteroskedasticity. What do you find ? Comment on the results. Add `as.factor(country)` in the regression and comment on the results.
